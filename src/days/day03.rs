...
impl Day for Day03 {
    fn part1(&self, input: &str) -> Result<String, &str> {
        ...
        for dir_size_str in wire2_split.clone() {
            let length: i32 = dir_size_str[1..].parse().unwrap();
            let next_pos = match &dir_size_str[..1] {
                "U" => {
                    for y in previous_pos.1..=previous_pos.1 + length {
        ...
        let intersections: HashSet<Point> = wire1_positions
            .intersection(&wire2_positions)
            .copied()
            .collect();
        let min_distance = intersections
            .iter()
            .map(|point| abs(point.0) + abs(point.1))
            .min()
            .unwrap();
        Ok(format!(
    ...
    fn part2(&self, input: &str) -> Result<String, &str> {
        ...
        for dir_size_str in wire1_split {
            let length = &dir_size_str[1..].parse::<i32>().unwrap();
            let next_pos =
                get_positions_for_move(previous_pos, (&dir_size_str[..1], *length), &mut |point| {
                    wire_1_length_count += 1;
                    if intersections.contains(&point) {
                        // && !wire1_intersection_distance.contains_key(&point){
        ...
        for dir_size_str in wire2_split {
            ...
            let next_pos =
                get_positions_for_move(previous_pos, (&dir_size_str[..1], *length), &mut |point| {
                    ...
                    if intersections.contains(&point) {
                        // && !wire2_intersection_distance.contains_key(&point) {
        ...
        Ok(format!(
            "Combined wire distance to first intersection {}",
            a.min().unwrap()
        ))
    }
}

fn get_positions_for_move(
    from: Point,
    dir_length: (&str, i32),
